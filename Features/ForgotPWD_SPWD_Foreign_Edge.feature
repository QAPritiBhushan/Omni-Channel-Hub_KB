Feature: Forgot Password - Login Password for Foreign Customer
  ...
  Scenario: Verifying if user is displayed with warning messages upon clicking on Continue button without providing username and registration number
     Then I click on Continue button without entering username and passport number
  ...
  Scenario: Verifying if user is able to successfully reset the login password for a Foreign Customer
    ...
    When I set the new login password for a Foreign customer and proceed
